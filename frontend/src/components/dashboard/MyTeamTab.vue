  ...
  <template v-else>
    <div class="tp-card">
      <div style="display:flex; justify-content:space-between; flex-wrap:wrap; gap:10px;">
        ...
        <div><div class="tp-note">{{ t("swaps_left") }}</div><div class="display" style="font-size:30px;">{{ swapsLeft }}<span style="font-size:16px;color:var(--text-muted);">/{{ store.poule.totalSwaps }}</span></div></div>
      </div>
    </div>
    ...
    <div class="tp-card">
      ...
      <div class="tp-rider-grid">
        <div v-for="id in roster" :key="id" class="tp-rider-card" style="cursor:default;">
          ...
          <button class="tp-btn secondary small" style="margin-left:8px;" :disabled="swapsLeft <= 0" @click="swapOutId = id">{{ t("btn_swap") }}</button>
        </div>
      </div>
